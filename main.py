...
from func.SendMsg import SendMsg, scrapp_ccs, chk_bin
from func.dates import APROVED, DEAD, TEXT

# Cargar variables del archivo .env
load_dotenv()

API_HASD = os.getenv("API_HASH")
API_ID = int(os.getenv("API_ID"))
NUMERO = os.getenv("PHONE_NUMBER")

# Configurar el cliente de Telethon
...
async def handler(event):
    ...
    class EventMsg:
        ...
        def Run(self):
            if re.search(self.RexGex, self.getMsg) and any(live in self.getMsg for live in DEAD):
                scrapp_ccs(self.ScrCss)
                return '[⌁] - Dead ❌'
            else:
                try:
                    if re.search(self.RexGex, self.getMsg) and any(live in self.getMsg for live in APROVED):
